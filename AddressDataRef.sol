 ...
 */
contract AddressDataRef is Context, ERC721 {
    ...
    mapping (uint256 => mapping (address => bytes32)) internal _metadataLocation;
    mapping (uint256 => mapping (address => bytes32)) internal _tokenDataLocation;
    mapping (uint256 => address[]) internal _tokenAddresses;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory data = "";
        for(uint256 i=0;i<_tokenAddresses[tokenId].length;i++)
           string(abi.encode(data, _metadataLocation[tokenId][_tokenAddresses[tokenId][i]], _tokenDataLocation[tokenId][_tokenAddresses[tokenId][i]], owner)); // return data pairs of all addresses for all tokenIds
           
        return data;
    }
    /* @dev See if address has access to data and get it's referenced data location */
    function tokenURIAccess(uint256 tokenId) public  view returns (address[] memory) {
    ...
    */
    function addAddressDataFor(uint256 tokenId, address to, bytes32 metadataSwarmLocation, bytes32 tokenDataSwarmLocation) public {
        address owner = ERC721.ownerOf(tokenId);
        require(msg.sender==owner, "!owner "); // not owner
        require(_tokenDataLocation[tokenId][to]==0, "Recorded "); // should it be immutable ? 
        
        _tokenDataLocation[tokenId][to] = tokenDataSwarmLocation;
        _metadataLocation[tokenId][to]  = metadataSwarmLocation;
        _tokenAddresses[tokenId].push(to); 
    }
}
